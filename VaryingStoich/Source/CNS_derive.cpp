                  ...
                  Real /*time*/, const int* /*bcrec*/, int /*level*/)
{
    ...
    [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept 
    {
        ...
        yder(i,j,k,dcomp) = wi * dxinv[0] 
                          * ( (dat(ip,j,k,URHOY)/dat(ip,j,k,URHO)) - (dat(im,j,k,URHOY)/dat(im,j,k,URHO)) );
        yder(i,j,k,dcomp+1) = wj * dxinv[1] 
                          * ( (dat(i,jp,k,URHOY)/dat(i,jp,k,URHO)) - (dat(i,jm,k,URHOY)/dat(i,jm,k,URHO)) );
#if AMREX_SPACEDIM==3
        yder(i,j,k,dcomp+2) = wk * dxinv[2] 
                          * ( (dat(i,j,kp,URHOY)/dat(i,j,kp,URHO)) - (dat(i,j,km,URHOY)/dat(i,j,km,URHO)) );
#endif
    });
}
