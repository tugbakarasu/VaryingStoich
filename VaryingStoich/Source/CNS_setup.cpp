...
CNS::variableSetUp ()
{
    ...
    std::cout << "cnt: " << cnt << ", name: " << name[cnt] << std::endl;
...
std::cout << "cnt: " << cnt << ", name: " << name[cnt] << std::endl;
    ...
    cnt++; set_scalar_bc(bc, phys_bc); bcs[cnt] = bc; name[cnt] = "phi"; // Equivalence ratio
    std::cout << "cnt: " << cnt << ", name: " << name[cnt] << std::endl;


    cnt++; set_scalar_bc(bc, phys_bc); bcs[cnt] = bc; name[cnt] = "rhoY_fuel"; // Fuel
    std::cout << "cnt: " << cnt << ", name: " << name[cnt] << std::endl;

    cnt++; set_scalar_bc(bc, phys_bc); bcs[cnt] = bc; name[cnt] = "rhoY_oxid"; // Oxidizer
    std::cout << "cnt: " << cnt << ", name: " << name[cnt] << std::endl;

    cnt++; set_scalar_bc(bc, phys_bc); bcs[cnt] = bc; name[cnt] = "rhoY_prod"; // Product
    std::cout << "cnt: " << cnt << ", name: " << name[cnt] << std::endl;

    cnt++; set_scalar_bc(bc, phys_bc); bcs[cnt] = bc; name[cnt] = "unburned_fuel";
    std::cout << "cnt: " << cnt << ", name: " << name[cnt] << std::endl;

    cnt++; set_scalar_bc(bc, phys_bc); bcs[cnt] = bc; name[cnt] = "unburned_oxidizer";
    std::cout << "cnt: " << cnt << ", name: " << name[cnt] << std::endl;

    cnt++; set_scalar_bc(bc, phys_bc); bcs[cnt] = bc; name[cnt] = "progress_var";
    std::cout << "cnt: " << cnt << ", name: " << name[cnt] << std::endl;
	...
	// Gradient of fuel mass fraction
    amrex::Vector<std::string> var_names_gfuel(AMREX_SPACEDIM);
    var_names_gfuel[0] = "dYfuel_dx";
    var_names_gfuel[1] = "dYfuel_dy";
 #if AMREX_SPACEDIM == 3
var_names_gfuel[2] = "dYfuel_dz";
#endif
    derive_lst.add("dY_fuel", IndexType::TheCellType(), AMREX_SPACEDIM,
                   var_names_gfuel, cns_derYderiv, grow_box_by_one);
    derive_lst.addComponent("dY_fuel", desc_lst, State_Type, URHOY_FUEL, 1);

     // Gradient of oxidizer mass fraction
    amrex::Vector<std::string> var_names_goxid(AMREX_SPACEDIM);
    var_names_goxid[0] = "dYoxid_dx";
    var_names_goxid[1] = "dYoxid_dy";
#if AMREX_SPACEDIM == 3
var_names_goxid[2] = "dYoxid_dz";
#endif
    derive_lst.add("dY_oxid", IndexType::TheCellType(), AMREX_SPACEDIM,
                   var_names_goxid, cns_derYderiv, grow_box_by_one);
    derive_lst.addComponent("dY_oxid", desc_lst, State_Type, URHOY_OXID, 1);

    // Gradient of product mass fraction
    amrex::Vector<std::string> var_names_gprod(AMREX_SPACEDIM);
    var_names_gprod[0] = "dYprod_dx";
    var_names_gprod[1] = "dYprod_dy";
#if AMREX_SPACEDIM == 3
var_names_gprod[2] = "dYprod_dz";
#endif
     derive_lst.add("dY_prod", IndexType::TheCellType(), AMREX_SPACEDIM,
                     var_names_gprod, cns_derYderiv, grow_box_by_one);
     derive_lst.addComponent("dY_prod", desc_lst, State_Type, URHOY_PROD, 1);
