...
extern "C" {
	...
	pp.query("OF_st", CNS::h_prob_parm->OF_st);
        ...
        pp.query("rho_rich", CNS::h_prob_parm->rich.rhot);
	pp.query("Y_fuel_rich", CNS::h_prob_parm->rich.Y_fuel);
	pp.query("Y_oxidizer_rich", CNS::h_prob_parm->rich.Y_oxid);
	pp.query("Y_product_rich", CNS::h_prob_parm->rich.Y_prod);

	pp.query("Y_fuel_lean", CNS::h_prob_parm->lean.Y_fuel);
	pp.query("Y_oxidizer_lean", CNS::h_prob_parm->lean.Y_oxid);
	pp.query("Y_product_lean", CNS::h_prob_parm->lean.Y_prod);
        pp.query("rho_lean", CNS::h_prob_parm->lean.rhot);

        //Print() << "Phi from input file: " << CNS::h_prob_parm->equivalence_ratio << "\n";
