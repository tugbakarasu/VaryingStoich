...
void cns_derYF (const Box& bx, FArrayBox& YFfab, int dcomp, int /*ncomp*/,
                  const FArrayBox& datfab, const Geometry& /*geomdata*/,
                  Real /*time*/, const int* /*bcrec*/, int /*level*/)
{
    auto const dat = datfab.array();
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
    {
        yF(i,j,k,dcomp) = dat(i,j,k,5)/dat(i,j,k,0);
    });
}

void cns_derYO (const Box& bx, FArrayBox& YOfab, int dcomp, int /*ncomp*/,
                  const FArrayBox& datfab, const Geometry& /*geomdata*/,
                  Real /*time*/, const int* /*bcrec*/, int /*level*/)
{
    auto const dat = datfab.array();
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
    {
        yO(i,j,k,dcomp) = dat(i,j,k,6)/dat(i,j,k,0);
    });
}

void cns_derYP (const Box& bx, FArrayBox& YPfab, int dcomp, int /*ncomp*/,
                  const FArrayBox& datfab, const Geometry& /*geomdata*/,
                  Real /*time*/, const int* /*bcrec*/, int /*level*/)
{
    auto const dat = datfab.array();
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
    {
        yP(i,j,k,dcomp) = dat(i,j,k,7)/dat(i,j,k,0);
    });
}
...
void cns_derYFderiv (const Box& bx, FArrayBox& yFderfab, int dcomp, int /*ncomp*/,
                  const FArrayBox& datfab, const Geometry& geomdata,
                  Real /*time*/, const int* /*bcrec*/, int /*level*/)
{

    auto const dat = datfab.array();
    auto       yFder = yFderfab.array();
    const auto dxinv = geomdata.InvCellSizeArray();

    const auto& flag_fab = amrex::getEBCellFlagFab(datfab);
    const auto& typ = flag_fab.getType(bx);
    if (typ == amrex::FabType::covered) {
        yFderfab.setVal<amrex::RunOn::Device>(0.0, bx);
        return;
    }
    const auto& flags = flag_fab.const_array();
    const bool all_regular = typ == amrex::FabType::regular;
    ...
    [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept 
    {
        ...
        yFder(i,j,k,dcomp) = wi * dxinv[0] 
                          * ( (dat(ip,j,k,URHOY_F)/dat(ip,j,k,URHO)) - (dat(im,j,k,URHOY_F)/dat(im,j,k,URHO)) );
        yFder(i,j,k,dcomp+1) = wj * dxinv[1] 
                          * ( (dat(i,jp,k,URHOY_F)/dat(i,jp,k,URHO)) - (dat(i,jm,k,URHOY_F)/dat(i,jm,k,URHO)) );
#if AMREX_SPACEDIM==3
        yFder(i,j,k,dcomp+2) = wk * dxinv[2] 
                          * ( (dat(i,j,kp,URHOY_F)/dat(i,j,kp,URHO)) - (dat(i,j,km,URHOY_F)/dat(i,j,km,URHO)) );
#endif
    });
}

void cns_derYOderiv (const Box& bx, FArrayBox& yOderfab, int dcomp, int /*ncomp*/,
                  const FArrayBox& datfab, const Geometry& geomdata,
                  Real /*time*/, const int* /*bcrec*/, int /*level*/)
{

    auto const dat = datfab.array();
    auto       yOder = yOderfab.array();
    const auto dxinv = geomdata.InvCellSizeArray();

    const auto& flag_fab = amrex::getEBCellFlagFab(datfab);
    const auto& typ = flag_fab.getType(bx);
    if (typ == amrex::FabType::covered) {
        yOderfab.setVal<amrex::RunOn::Device>(0.0, bx);
        return;
    }
    const auto& flags = flag_fab.const_array();
    const bool all_regular = typ == amrex::FabType::regular;
    ...
    [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept 
    {
        ...
        yOder(i,j,k,dcomp) = wi * dxinv[0] 
                          * ( (dat(ip,j,k,URHOY_O)/dat(ip,j,k,URHO)) - (dat(im,j,k,URHOY_O)/dat(im,j,k,URHO)) );
        yOder(i,j,k,dcomp+1) = wj * dxinv[1] 
                          * ( (dat(i,jp,k,URHOY_O)/dat(i,jp,k,URHO)) - (dat(i,jm,k,URHOY_O)/dat(i,jm,k,URHO)) );
#if AMREX_SPACEDIM==3
        yOder(i,j,k,dcomp+2) = wk * dxinv[2] 
                          * ( (dat(i,j,kp,URHOY_O)/dat(i,j,kp,URHO)) - (dat(i,j,km,URHOY_O)/dat(i,j,km,URHO)) );
#endif
    });
}

void cns_derYPderiv (const Box& bx, FArrayBox& yPderfab, int dcomp, int /*ncomp*/,
                  const FArrayBox& datfab, const Geometry& geomdata,
                  Real /*time*/, const int* /*bcrec*/, int /*level*/)
{

    auto const dat = datfab.array();
    auto       yPder = yPderfab.array();
    const auto dxinv = geomdata.InvCellSizeArray();

    const auto& flag_fab = amrex::getEBCellFlagFab(datfab);
    const auto& typ = flag_fab.getType(bx);
    if (typ == amrex::FabType::covered) {
        yPderfab.setVal<amrex::RunOn::Device>(0.0, bx);
        return;
    }
    const auto& flags = flag_fab.const_array();
    const bool all_regular = typ == amrex::FabType::regular;
    ...
    [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept 
    {
        ...
        yPder(i,j,k,dcomp) = wi * dxinv[0] 
                          * ( (dat(ip,j,k,URHOY_P)/dat(ip,j,k,URHO)) - (dat(im,j,k,URHOY_P)/dat(im,j,k,URHO)) );
        yPder(i,j,k,dcomp+1) = wj * dxinv[1] 
                          * ( (dat(i,jp,k,URHOY_P)/dat(i,jp,k,URHO)) - (dat(i,jm,k,URHOY_P)/dat(i,jm,k,URHO)) );
#if AMREX_SPACEDIM==3
        yPder(i,j,k,dcomp+2) = wk * dxinv[2] 
                          * ( (dat(i,j,kp,URHOY_P)/dat(i,j,kp,URHO)) - (dat(i,j,km,URHOY_P)/dat(i,j,km,URHO)) );
#endif
    });
}
