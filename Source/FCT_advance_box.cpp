                       ...
                       Real dt, int rk)
{
    ...
    if(do_react == 1){
        ...
        [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
        {
            ...
            AMREX_D_TERM(
            udi[0](i,j,k,URHOY) = amrex::min(udi[0](i,j,k,URHO),udi[0](i,j,k,URHOY) + dt*omegarho);,
            udi[1](i,j,k,URHOY) = amrex::min(udi[1](i,j,k,URHO),udi[1](i,j,k,URHOY) + dt*omegarho);,
            udi[2](i,j,k,URHOY) = amrex::min(udi[2](i,j,k,URHO),udi[2](i,j,k,URHOY) + dt*omegarho);
            );

            AMREX_D_TERM(
            udi[0](i,j,k,URHOY) = amrex::max(0.0,udi[0](i,j,k,URHOY));,
            udi[1](i,j,k,URHOY) = amrex::max(0.0,udi[1](i,j,k,URHOY));,
            udi[2](i,j,k,URHOY) = amrex::max(0.0,udi[2](i,j,k,URHOY));
            );

            udfab(i,j,k,UEDEN) = udfab(i,j,k,UEDEN) - dt*omegarho*lparm->q_dim;

            udfab(i,j,k,URHOY) = amrex::min(udfab(i,j,k,URHO),udfab(i,j,k,URHOY) + dt*omegarho);
            udfab(i,j,k,URHOY) = amrex::max(0.0,udfab(i,j,k,URHOY));

            AMREX_D_TERM(
            utr[0](i,j,k,URHOY) = amrex::max(0.0,amrex::min(utr[0](i,j,k,URHO),utr[0](i,j,k,URHOY)));,
            utr[1](i,j,k,URHOY) = amrex::max(0.0,amrex::min(utr[1](i,j,k,URHO),utr[1](i,j,k,URHOY)));,
            utr[2](i,j,k,URHOY) = amrex::max(0.0,amrex::min(utr[2](i,j,k,URHO),utr[2](i,j,k,URHOY)));
            );
        });
    }
