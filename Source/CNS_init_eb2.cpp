                ...
                const int max_coarsening_level)
{
...
#endif
    {
        if(geom_type == "prism"){
            ...
            Real theta, height;
            Vector<Real> pl1pt;
            pp.getarr("start_point", pl1pt);
            pp.get("theta", theta);
            pp.get("height", height);

            // Covert theta to radians
            theta *= PI / 180.0; 
            Real nx = cos(0.5*PI + theta);
            Real ny = sin(0.5*PI + theta);

            Real length = 0.5*height / tan(theta);

            auto prism = EB2::makeIntersection(EB2::PlaneIF({pl1pt[0],pl1pt[1]},{nx,ny},false),
                                          EB2::PlaneIF({pl1pt[0], pl1pt[1]},{nx,-ny},false),
                                          EB2::PlaneIF({pl1pt[0]+height, pl1pt[1]},{1.,0.},false));

            auto gshop = EB2::makeShop(prism);
            EB2::Build(gshop, geom, max_coarsening_level, max_coarsening_level, 8);
        }else if(geom_type == "user_defined"){
            init_eb_geometry(geom, max_coarsening_level);
        }else{
            EB2::Build(geom, max_coarsening_level, max_coarsening_level, 8);
        }
